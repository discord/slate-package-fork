...
var Text = function (_Record) {
  ...
  createClass(Text, [{
    ...
    value: function getLeaves(decorations) {
      ...
      var leaves = [{ text: text, marks: marks, decorations: [] }];
...
var Value = function (_Record) {
  ...
  createClass(Value, [{
    ...
    value: function insertText(path, offset, text) {
      ...
      value = value.mapPoints(function (point) {
        if (point.key === node.key && point.offset >= offset) {
          return point.setOffset(Math.min(node.text.length, point.offset + text.length));
        } else {
          return point;
        }
      }, true);
    ...
    key: 'mapRanges',
    value: function mapRanges(iterator, skipNormalize) {
      var value = this;
      var _value17 = value,
          document = _value17.document,
          oldSelection = _value17.selection,
          annotations = _value17.annotations;


      var newSelection = oldSelection.isSet ? iterator(oldSelection) : oldSelection;
      if (!newSelection) newSelection = oldSelection.unset();
      if (newSelection !== oldSelection && !skipNormalize) newSelection = document.createSelection(newSelection);
    ...
    key: 'mapPoints',
    value: function mapPoints(iterator, skipNormalize) {
...
Commands$1.deleteCharForwardAtRange = function (editor, range) {
  ...
  if (startBlock == null) {
    var node = document.getNode(start.path);

    if (node != null && node.object === 'block' || node.object === 'document') {
      startBlock = node;
    }
  }
  ...
  var offset = startBlock.getOffset(start.key);
  var o = offset + start.offset;
  var _startBlock = startBlock,
      text = _startBlock.text;
...
Commands$1.deleteWordBackwardAtRange = function (editor, range) {
  ...
  var remainingTextInNode = text.substring(offset, offset + start.offset);
...
Commands$1.deleteWordForwardAtRange = function (editor, range) {
  ...
  var startBlock = document.getClosestBlock(start.path);
  var offset = startBlock.getOffset(start.key);
  var node = startBlock.getNode(start.key);
  var o = offset + start.offset;
  var text = startBlock.text;


  var remainingTextInNode = node.text.substring(start.offset);

  if (remainingTextInNode.trim().length === 0) {
    var nodeIndex = startBlock.nodes.indexOf(node);

    if (nodeIndex + 1 < startBlock.nodes.size) {
      var nextNode = startBlock.nodes.get(nodeIndex + 1);
...
Commands$4.ensureSelection = function (editor) {
  var _editor$value = editor.value,
      selection = _editor$value.selection,
      document = _editor$value.document;


  if (selection.start == null || selection.end == null || selection.start.isUnset || selection.end.isUnset) {
...
var Editor = function () {
  ...
  createClass(Editor, [{
    ...
    key: 'fastInsertText',
    value: function fastInsertText(path, offset, text, marks) {
      var _this2 = this;
    ...
    for (var _iterator = Classes[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      try {
        for (var _iterator3 = Object.getOwnPropertyNames(Interface.prototype)[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
          ...
          try {
            if (desc == null) {
              console.log("Property prototype descriptor was undefined for: " + _name);

              Class.prototype[_name] = Interface.prototype[_name];
            } else {
...
mixin(ModelInterface, [Annotation, Block, Decoration, Document, Inline, Leaf, Mark, Node, Operation, Point, Range, Selection, Text, Value]);
...
export { Annotation, Block, Change, Data, Decoration, Document, Editor, Inline, KeyUtils, Leaf, Mark, Node, Operation, PathUtils, Point, Range, resetMemoization, Selection, Text, TextUtils, useMemoization, Value };
